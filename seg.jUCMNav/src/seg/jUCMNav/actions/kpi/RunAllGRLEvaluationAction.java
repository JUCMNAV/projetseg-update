        ...
        if (sel.getGroup() != null || sel.getAllModel().size() > 1) {
            ...
            if(evalTable.size() > 0) {
                ...
                for (Iterator iter = grlSpec.getActors().iterator(); iter.hasNext();) {
                    ...
                    MetadataHelper.addMetaData(grlSpec.getUrnspec(), actor, "_trend", Integer.valueOf(trend).toString()); //$NON-NLS-1$
                }
                ...
                for (Iterator i = grlSpec.getIntElements().iterator(); i.hasNext();) {
                    ...
                    MetadataHelper.addMetaData(grlSpec.getUrnspec(), element, "_trend", Integer.valueOf(trend).toString()); //$NON-NLS-1$
                }
            }
