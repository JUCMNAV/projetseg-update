...
public class CSMDupConnectionList {
    ...
    ArrayList<CSMDupConnection> connList = new ArrayList<CSMDupConnection>(1000); // TODO: wave limitation
    ...
    public void remove(CSMDupNode source, PathNode target) {
        for (int i = 0; i < connList.size(); i++) {
            if (connList.get(i).getSourceStr() == source.getId() && connList.get(i).getTargetStr() == target.getId()) {
                connList.remove(i);
            }
        }
    }
    ...
    public CSMDupNode getSourceForTarget(PathNode target) {
        for (int i = 0; i < connList.size(); i++) {
            if (connList.get(i).getCSMTarget() != null && (connList.get(i).getCSMTarget()).getNode() != null) {
                if ((connList.get(i).getCSMTarget()).getNode() == target) {
                    CSMDupNode source = connList.get(i).getCSMSource();
    ...
    public CSMDupNode getTargetforSourceTowardNode(PathNode source, PathNode destination) {
        for (int i = 0; i < connList.size(); i++) {
            if ((connList.get(i).getCSMSource()).getId() == source.getId()) {
                CSMDupNode target = connList.get(i).getCSMTarget();
                if (destination == null) {
                    return target;
    ...
    public CSMDupNode getTargetForSourceTowardNode(String source, PathNode destination) {
        for (int i = 0; i < connList.size(); i++) {
            if ((connList.get(i).getCSMSource()).getId() == source) {
                CSMDupNode target = connList.get(i).getCSMTarget();
                if (destination == null) {
                    return target;
    ...
    public CSMDupNode getSourceForTargetTowardNode(PathNode target, PathNode destination) {
        for (int i = 0; i < connList.size(); i++) {
            if ((connList.get(i).getCSMTarget()).getId() == target.getId()) {
                CSMDupNode source = connList.get(i).getCSMSource();
                if (destination == null) {
                    return source;
    ...
    public CSMDupNode getSourceForTargetTowardNode(String target, PathNode destination) {
        for (int i = 0; i < connList.size(); i++) {
            if ((connList.get(i).getCSMTarget()).getId() == target) {
                CSMDupNode source = connList.get(i).getCSMSource();
                if (destination == null) {
                    return source;
    ...
    public String getSourceForTarget(String target) {
        for (int i = 0; i < connList.size(); i++) {
            if (connList.get(i).getTargetStr() == target) {
                return connList.get(i).getSourceStr();
            }
        }
        return null;
    }
    ...
    public CSMDupNode getTargetForSource(PathNode source) {
        for (int i = 0; i < connList.size(); i++) {
            if (connList.get(i).getCSMSource() != null && (connList.get(i).getCSMSource()).getNode() != null) {
                if ((connList.get(i).getCSMSource()).getNode() == source) {
                    CSMDupNode target = connList.get(i).getCSMTarget();
                    return target;
                }
            }
    ...
    public String getTargetForSource(String source) {
        for (int i = 0; i < connList.size(); i++) {
            if (connList.get(i).getSourceStr() == source) {
                return connList.get(i).getTargetStr();
            }
        }
        return null;
    }
    ...
    public void getPrevEdgeList(String target, ArrayList<PathNode> sources) {
        for (int i = 0; i < connList.size(); i++) {
            String target_str = connList.get(i).getTargetStr();
            if (target_str == target) {
                String source_str = connList.get(i).getSourceStr();
                getPrevEdgeList(source_str, sources); // recursive call
                if (connList.get(i).getCSMSource().isPathNode()) {
                    sources.add((connList.get(i).getCSMSource()).getNode());
                } // if
            } // if
        } // for
    } // method
    ...
    public void getNextEdgeList(String source, ArrayList<PathNode> targets) {
        for (int i = 0; i < connList.size(); i++) {
            String source_str = connList.get(i).getSourceStr();
            if (source_str == source) {
                String target_str = connList.get(i).getTargetStr();
                getNextEdgeList(target_str, targets); // recursive call
                if (connList.get(i).getCSMTarget().isPathNode()) {
                    targets.add((connList.get(i).getCSMTarget()).getNode());
                } // if
            } // if
        } // for
    } // method
    ...
    // that source
    public ArrayList<String> getTargetFromList(String source) {
        ArrayList<String> targets = new ArrayList<String>(1000);
        for (int i = 0; i < connList.size(); i++) {
            if (connList.get(i).getSourceStr() == source) {
                targets.add(connList.get(i).getTargetStr());
            }
        }
        return targets;
    }
    ...
    public void printDupList() {
        ...
        for (int i = 0; i < connList.size(); i++) {
            ...
            System.out.println("Connection " + i + ": " + " Source: " + connList.get(i).getSourceStr() + sourceName + " Target: " //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                    + connList.get(i).getTargetStr() + targetName);
        }
    }
    ...
    public boolean existsConnectionForSource(CSMDupNode source) {
        for (int i = 0; i < connList.size(); i++) {
            String conn_source_id = ((connList.get(i))).getSourceStr();
