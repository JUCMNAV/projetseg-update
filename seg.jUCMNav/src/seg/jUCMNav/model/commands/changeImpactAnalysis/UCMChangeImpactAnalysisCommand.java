...
public class UCMChangeImpactAnalysisCommand extends Command implements JUCMNavCommand{
   ...
   */
	  public UCMChangeImpactAnalysisCommand(EObject Criterion,ArrayList<String> criterionVariables,int changeType){
	      ...
	      if(changeType == 2)
			//JOptionPane.showMessageDialog(null,"2. Deletion");
	        delType=true;
	      else
			//JOptionPane.showMessageDialog(null,"1. Addition/Modification");
			delType=false;
    ...
    public void runSlicingAlg()
    {
		...
		slicing.isorAndTimerCriterion= isorAndTimerCriterion;
    			...
    			if(specDiagram instanceof IURNDiagram)
    				for(Object comment :((IURNDiagram)specDiagram).getComments())
    					if(((Comment)comment).getFillColor() == "228,228,228"){
    						...
    						IURNDiagram graph  = null;
    				    	if(Criterion instanceof RespRef)
    					    	graph = (IURNDiagram)((RespRef) Criterion).getDiagram();//responsibility 
    				    	else if(Criterion instanceof StartPoint)
    				    		graph = (IURNDiagram)((StartPoint) Criterion).getDiagram(); // start point 
    				    	else if(Criterion instanceof NodeConnection)
    				    		graph = (IURNDiagram)((NodeConnection) Criterion).getDiagram(); // OrFork - AndFork - OrJoin - AndJoin  
