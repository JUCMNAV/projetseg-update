...
public class ForSlicingAlg {
    ...
    public boolean isorAndTimerCriterion = false; 
...
{    
	...
	if(Criterion instanceof NodeConnection)
	{
		...
		if(isorAndTimerCriterion){
			editPartList.setIsorAndTimerCriterion(true);
		}else
			visitedNodes.remove(0);
	}
						...
						if(!allImpactedElements.isEmpty()){
							for(EObject targetElem:allImpactedElements)
							{
								if(targetElem instanceof RespRef){
									...
									infoCommentUrlLinks += "\n- urnLink type : " + ((URNlink) urnLink).getType() + " | Element name: " +respp.getRespDef().getName()+
											"\n     => Source=" + strGetFromElem + " - Name: " + respp.getRespDef().getName() +
											" - UCMmap: " + ((ucm.map.UCMmap)((IURNDiagram)respp.getDiagram())).getName() +
											"\n     => Target=" + strGetToElem + " - " + resRef.getRespDef().getName() +
											" - UCMmap: " + ((ucm.map.UCMmap)((IURNDiagram)resRef.getDiagram())).getName()  + "\n";
									foundUrnLinks = true;
								}else if(targetElem instanceof IntentionalElementRef)
								{
									...
									infoCommentUrlLinks += "\n- urnLink type : " + ((URNlink) urnLink).getType() + 
											"\n     => Source=" + strGetFromElem + " - Name: " + respp.getRespDef().getName() +
											" - UCMmap: " + ((ucm.map.UCMmap)((IURNDiagram)respp.getDiagram())).getName() +
									...
									foundUrnLinks = true;		
										...
										foundUrnLinks = true;
								}else if(targetElem instanceof Component){
